...
# Коды ролей (совместимость с v1: 1..6 + расширение для контакт-центра)
class Roles:
    OP1 = 1
    OP2 = 2
    SP1 = 3
    ...
    SP3 = 5
    ADMIN = 6
    CALL_ADMIN = 7  # Администратор контакт-центра (новое в v2)
...
# Обратное отображение: имя группы -> код роли
GROUP_ROLE_MAP = {v: k for k, v in ROLE_GROUP_MAP.items()}

# Роли ТФОМС (административные и операторские линии фонда)
TFOMS_ROLES = {Roles.OP1, Roles.OP2, Roles.ADMIN, Roles.CALL_ADMIN}

# Роли СМО (страховые представители)
