...
Каждая колонка: key (техническое имя), label, group (шапка-группа колонок),
sortable (поддержка сортировки по реквизиту). Группировка колонок визуально
объединяется общей шапкой (двухуровневый thead).
"""
