...
До развёртывания адрес БД должен отвечать как с хоста, так и из контейнерной сети:
...
1. Остановить все старые writers и сделать полный `pg_dump` PostgreSQL 17.
2. Проверить восстановление копии и выполнить на ней полный запуск нового image.
3. Установить `ALLOW_LEGACY_INPLACE_UPGRADE=true` только на первый запуск.
4. Выполнить `docker compose up -d --no-build` и проверить завершение `migrate`.
