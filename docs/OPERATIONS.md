...
Для промышленного единого репозитория задайте `ACCOUNT_REPOSITORY_BACKEND=http`, HTTPS-базу в `ACCOUNT_REPOSITORY_URL`, отдельный Bearer-секрет длиной не менее 32 символов в `ACCOUNT_REPOSITORY_TOKEN` и таймаут 1–30 секунд. Портал выполняет `GET {URL}/employees/{guid}` и ожидает JSON с полями `guid`, `username`, `org`, `is_active`, необязательными `first_name`, `last_name`, `job_title` и массивом канонических `roles`. Профиль синхронизируется в локальную shadow-учетную запись при token login; пароль, staff и superuser из внешнего ответа не принимаются. Несовпадающий GUID, неизвестная организация/роль, конфликт username, сетевой сбой и невалидный JSON приводят к отказу во входе. `local` оставлен только для автономного контура и разработки.
...
приватный timestamp-каталог, записывает в `MANIFEST` точную Git-ревизию application image и
версию PostgreSQL и возобновляет сервисы даже при ошибке копирования:

```bash
sh scripts/backup_release.sh ./backups
```

Backup и restore используют один неблокирующий host lock
`/tmp/ofoms-ejournal-backup-restore.lock`: второй параллельный запуск завершается до обращения к
контейнерам. Для нескольких независимых инсталляций на одном хосте задайте каждой одинаковый
внутри своей инсталляции и уникальный между инсталляциями абсолютный путь через
`BACKUP_RESTORE_LOCK_FILE`. На хосте требуется `flock` из util-linux.

До завершения копия хранится в process-specific каталоге `.partial.<pid>`. Скрипт публикует
timestamp-каталог одним rename только после формирования всех artifacts и checksum-набора;
при любой ошибке partial-каталог удаляется. Поэтому автоматическая off-host выгрузка должна
выбирать только каталоги формата `YYYYMMDDTHHMMSSZ` и никогда не подбирать `.partial.*`.
Backup требует, чтобы `web` и `scheduler` исходно были запущены; иначе он завершается до создания
staging. Даже при частичном отказе остановки cleanup пытается запустить оба исходных container ID
и удалить staging, не маскируя ошибку восстановления writers.
...
Restore до удаления данных проверяет полный список artifacts, SHA-256, версию формата manifest,
имя БД, читаемость обоих tar-архивов и каталога PostgreSQL dump. Ревизия работающего `web` image
обязана совпасть с `VCS_REF` копии: сначала разверните указанный в `MANIFEST` immutable image,
затем запускайте восстановление. Это исключает случайный запуск старой схемы на новом коде или
новой схемы на старом коде. При ошибке после начала destructive phase `web/scheduler` намеренно
остаются остановленными: сначала устраните причину и повторите полное восстановление. После
загрузки данных скрипт применяет миграции выбранного image и только затем запускает writers.
