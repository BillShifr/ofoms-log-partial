...
|---|---|---|---|---|---|---|---|
| Реестр, карточка, маркировка, поиск и сортировка | ТЗ 2.1 | ТФОМС, СМО | journal | models/views/templates/print list | Django tests | Печать списка реализована; вместо infinite scroll используется доступная серверная пагинация | реализовано |
...
| Эксплуатационная документация и видеоролики | ТЗ 3.7 | Admin, все пользователи | system/docs | защищённая загрузка/каталог/download | upload, ACL и validator tests | Документы до 20 МБ; MP4/WebM/OGV до 200 МБ; исполняемые форматы запрещены | реализовано |
